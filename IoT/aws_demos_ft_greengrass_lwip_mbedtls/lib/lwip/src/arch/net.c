...
#define NET_ETHERNET_INPUT_TASK_STACKSIZE 	(512+256) // Fix crash issue
 ...
 */
sys_thread_t g_task_ethernet_input;
#endif
...
static void net_timeout(void *arg)
{
	...
	sys_timeout(500, net_timeout, arg);
}
...
void net_tick(void *pvParameters)
{
	...
	while (1)
	{
		...
		if (xTaskNotifyWait(0, (uint32_t)1, &ulNotifiedValue, pdMS_TO_TICKS(500)))
		{
