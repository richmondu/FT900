...
#if 0
void sdcard_test_write(const char* filename);
void sdcard_test_read(const char* filename);
...
void sdcard_test_read(const char* filename)
{
    ...
    FRESULT res = sdcard_open(&f, filename, 0, 1);
    if (res == FR_OK) {
        do {
        	sdcard_read(&f, buffer, 128, &read);
...
void sdcard_test_write(const char* filename)
{
    ...
    FRESULT res = sdcard_open(&f, filename, 1, 0);
    if (res == FR_OK) {
        ...
        while(towrite) {
        	sdcard_write(&f, testdata, towrite, &written);
...
int sdcard_open(FIL* f, const char* filename, int bWrite, int bRead) {
    ...
    {
        if (bWrite) {
            //uart_puts(UART0, "file already exists. Deleting\r\n");
            //res = f_unlink(filename);
            ASSERT_P(FR_OK, f_unlink(filename), "Problem deleting file");
        }
        else {
            //uart_puts(UART0, "file exists.\r\n");
        }
    }

    /* Open the file and dump out the contents */
    if (bWrite & bRead) {
        res = f_open(f, filename, FA_WRITE | FA_CREATE_NEW | FA_READ);
        ASSERT_P(FR_OK, res, "Problem creating file");
    }
    else if (bWrite) {
        //uart_puts(UART0, "Opening file for writing\r\n");
        res = f_open(f, filename, FA_WRITE | FA_CREATE_NEW);
        ASSERT_P(FR_OK, res, "Problem creating file");
    }
    else if (bRead){
        //uart_puts(UART0, "Opening file for reading\r\n\r\n");
        res = f_open(f, filename, FA_READ);
        ASSERT_P(FR_OK, res, "Problem reading file");
    }
